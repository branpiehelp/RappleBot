...
{
	this->servo_obj[sel].xs_wt((signed char)EEPROM.read(sel));
	return this->servo_obj[sel].yg_wt();
}
void RappleBot::set_servo_trim(int sel, signed char a)
{
	this->servo_obj[sel].xs_wt(a);
	EEPROM.write(sel, a);
}
...
{
	for(int i=0;i<MAX_SERVO_NUM;i++){
		this->servo_obj[i].ka(servo_pin[i]);
	}  
